...
class _irt(object):
    ...
    def fit_priors(self, *args):
        ...
        aj = tf.Variable(   
            np.log(np.exp(np.ones((n_items, 1))) - 1), #np.random.normal(0, 1, size=(n_items, 1)), #clip
            #np.ones((n_items, 1)) * 2.002374,
            trainable=True, dtype=tf.float32
        )
        ...
        for _ in tqdm(range(epochs)):
            ...
            if t % 1000 == 0:
                print(tf.math.sigmoid(thi).numpy().flatten(), 
                tf.math.sigmoid(delj[16]).numpy().flatten(), 
                tf.math.softplus(aj[16]), tf.math.tanh(bj[16]))
    ...
    def fit_three(self, *args):
        ...
        self
        """
        (queue, X, n_respondents, n_items, epochs, lr, random_seed, n_inits, tol, thetas) = args
        ...
        if thetas is None:
            ...
            thi = tf.Variable(
                np.log(-abil / (abil - 1)).reshape((1, n_respondents)), # Changed init there from random.normal to same as Beta4
                #np.random.normal(0,1, size=(1, n_respondents)), 
                trainable=True, dtype=tf.float32
            )
        else:
            ln_thetas = np.log(thetas / (1 - thetas))
            thi = tf.Variable(
                ln_thetas.reshape((1, n_respondents)),
                trainable=False, dtype=tf.float32,
            )
        
        diff = 1 - np.mean(X, axis=1)
        delj = tf.Variable(
            np.log(-diff / (diff - 1)).reshape((n_items, 1)), # Changed init there from random.normal to same as Beta4
            #np.random.normal(0,1, size=(n_items, 1)), 
            trainable=True, dtype=tf.float32
        )
        ...
        cor[np.isnan(cor)] = 0
        aj = tf.Variable(   
                np.log((-cor - 1) / (cor - 1)) / 2, #np.random.normal(cor,1, size=(n_items, 1)), # Changed init there from random.normal to same as Beta4 
                trainable=True, dtype=tf.float32
            )
        # else:
        #     aj = tf.Variable(   
...
class Beta3(_viz,_irt):
    ...
    def fit(self, X, y=None):
        ...
        args = (
            ...
            self.n_items,
            self.epochs, self.lr, self.n_seed, 
            self.n_inits, self.tol, self.thetas
        )
