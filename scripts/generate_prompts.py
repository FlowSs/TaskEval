...
if __name__ == '__main__':
  ...
  for ind in tqdm.tqdm(range(0, len(dataset))):
      ...
      signature, function, prompt = preprocess_prompt(dataset_name, dataset.iloc[ind])
      ...
      with open(os.path.join('..', '..', 'data', args.datset.lower(), file_to_save), 'w') as f:
        json.dump(dat, f)

      df = pd.read_json(os.path.join('..', '..', 'data', args.datset.lower(), file_to_save))
      df.to_csv(os.path.join('..', '..', 'data', args.datset.lower(), file_to_save.split('.json')[0] + '.csv'), encoding='utf-8', index=False)