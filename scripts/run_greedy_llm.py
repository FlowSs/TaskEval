...
if __name__ == '__main__':
   ...
   dat = pd.read_csv(os.path.join('..', 'data', f'{args.dataset}', file_data), \
   ...
   file_output = f'results_{args.dataset}_{model_name}_greedy.json'
   print(f"Running on dataset: {args.dataset} and model: {model_name} in Greedy Mode")

   if os.path.exists(os.path.join('..', 'data', f'{args.dataset}', 'raw', file_output)):
      with open(os.path.join('..', 'data', f'{args.dataset}', 'raw', file_output), 'r') as f:
   ...
   for task_id in tqdm.tqdm(dat.index.to_numpy()):
        ...
        with open(os.path.join('..', 'data', 'raw', args.dataset.lower(), file_output), 'w') as f:
