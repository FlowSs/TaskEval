...
for model in models:
	file_to_load = os.path.join('..', '..', 'data', 'classeval', 'post_test', f'results_ClassEval_{model}_eval.json')
...
for p, model in enumerate(models):
 ...
 file_to_save = f'results_ClassEval_{model}_sim_just_one.json'
 avg_dict = {}
 for ind_key in prompt.index.values:    
    avg_dict[int(ind_key)] = {}
    print("For Model {}, Running samples of task ======= {} ({}) =======".format(model, prompt.iloc[ind_key]['method_signature'], prompt.index[ind_key]))
    curr_key = 'ClassEval_' + prompt.iloc[ind_key]['class_id']
