...
class AutoTest:
        ...
        for (_, row), ind in zip(sampled_tasks.iterrows(), generated_codes):
            if 'greedy' in self.model_name:
                ...
                try:
                    ...
                    if 'gpt' in self.model_name or 'deepseek-chat' in self.model_name:
                        answer_code = re.findall(regex_code, code if code.startswith('```python') else '```python\n' + code)[0]
                        answer_code = [e for e in answer_code.split('\n') if 'import ' not in e and '@staticmethod' not in e]
    ...
    def run_unit_test(self, test_code, test_class):
        ...
        module = importlib.import_module(os.path.join(self.tmp_dir[:-1] + '.' + test_code))
    ...
    def test(self, code_num, test_code_name, test_class, code_list):
        ...
        if 'greedy' in self.model_name:
            result = None
        elif 'just_one' in self.model_name:
            result = {}
            result['original'] = []
        else:
            result = {}
        ...
        for i in range(code_num):
            ...
            try:
                ...
                if 'greedy' in self.model_name:
                    ...
                    result['original'].append((code_list['original'][i], True if len(res.errors) == 0 and len(res.failures) == 0 and res.testsRun != 0 else False))
                else:
                ...
                if 'greedy' in self.model_name:
                    ...
                    result['original'].append((code_list['original'][i], False))
                else:
                    result[level].append((code_list[level][i % 30], False))
            finally:
             if [x for x in os.getcwd().split(os.sep) if x.strip()][-1] == 'tmp_run':
              os.chdir('..')
